...
function Deploy:WriteDeploySettings()
	--Create the Settings file for deployment

	--Pull options from server


	--set options unique to remote factory
end
...
function Deploy:RemoteFactorySetup(address, port)
	...
	--local settingsFile = SysConfig.Settings_Manager.CreateRemoteSettings()
	...
	if self:RemoteFactoryBootEEPROM(nic,address,port) then
		...
		print(string.format("Response Code(%s) Sent to: %s on port %s", "0x0015", address, port))
	else
